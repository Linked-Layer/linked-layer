  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-6xl px-4 pb-24 pt-28 sm:px-6">
        <Link to="/" className="mb-6 inline-flex items-center gap-2 text-sm text-muted hover:text-white">
        ...
        <div className="grid gap-10 lg:grid-cols-[220px_1fr]">
          ...
          <article className="prose-linked max-w-none space-y-12">
            <Block id="abstract" title="1. Abstract">
              <p>
                {BRAND.name} is a token-gated shared-memory layer for teams and AI agents. It sits on top of a
                team's existing tools — Slack, documents, GitHub, trackers, calls and CRM — and distills them into a
                single, permission-aware <em>context graph</em>: the decisions, the reasoning behind them, the action
                items and their statuses. That memory is served to people through an “ask the company” interface and to
                autonomous agents through the Model Context Protocol (MCP) and a Context API with one call:
                <code> recall(query, scope)</code>. Access is gated by holding {BRAND.symbol}; external agents pay per
                call via x402, and fees fuel buyback &amp; burn.
              </p>
            </Block>
            ...
            <Block id="problem" title="2. The problem">
              <p>
                ...
                weeks reconstructing context; engineers re-litigate settled decisions; and the new wave of AI agents has
                no reliable, permissioned way to know <em>why</em> a team does what it does. Existing search tools index
                documents but don't capture decisions, rationale or cross-tool relationships — and none expose that
                context to agents natively.
              </p>
            </Block>

            <Block id="solution" title="3. The solution">
              <p>
                {BRAND.name} continuously ingests connected sources, mirrors their permissions, and builds a living
                graph of projects, people, decisions and threads. A distillation pipeline extracts durable facts and
                keeps them deduplicated and current. The result is a company's memory as a queryable, permission-bounded
                resource — usable by humans and machines through the same primitive.
              </p>
            </Block>
            ...
            <Block id="token" title="6. Token model">
              ...
              <p>
                ...
                first-class. Allocation: 50% liquidity, 20% community &amp; airdrops, 15% treasury (buyback &amp; burn),
                10% team (vested), 5% ecosystem.
              </p>
            </Block>

            <Block id="roadmap" title="7. Roadmap">
              <ol>
            ...
            <Block id="disclaimer" title="8. Disclaimer">
              <p className="text-sm text-muted">
                ...
                {" "}{BRAND.symbol} is a utility token; participate only in accordance with the laws of your jurisdiction.
