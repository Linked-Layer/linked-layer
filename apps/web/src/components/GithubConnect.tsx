        ...
        {!verified ? (
          ...
          <div className="space-y-3">
            ...
            {!repoOpts ? (
              ...
              <ul className="max-h-56 space-y-1 overflow-y-auto">
                {repoOpts.map((r) => (
                  <li key={r.fullName}>
                    <label className="flex cursor-pointer items-center gap-2 rounded-md border border-border bg-panel-2 px-2.5 py-1.5 text-xs text-ink hover:border-violet/50">
          ...
          <div className="space-y-3">
            ...
            <ul className="max-h-40 space-y-1 overflow-y-auto">
              {status!.repos.map((r) => (
                <li key={r} className="flex items-center gap-2 rounded-md border border-border bg-panel-2 px-2.5 py-1.5 text-xs text-ink">
          ...
          <div className="space-y-3">
            <p className="text-xs leading-relaxed text-muted">
              Authorize Linked to read your repos, then pick which ones to index. The chat answers from their code, READMEs,
            ...
            <button onClick={() => setUsePat(true)} className="w-full text-center text-[11px] text-muted hover:text-ink">
          ...
          <div className="space-y-3">
            ...
            <input
              ...
              autoComplete="off"
              className="w-full rounded-lg border border-border bg-panel-2 px-3 py-2 text-sm text-ink outline-none placeholder:text-muted focus:border-violet/60"
            />
            <textarea
              value={reposText}
              ...
              className="w-full resize-none rounded-lg border border-border bg-panel-2 px-3 py-2 font-mono text-xs text-ink outline-none placeholder:text-muted focus:border-violet/60"
            />
            ...
            {status?.oauthEnabled && (
              <button onClick={() => setUsePat(false)} className="w-full text-center text-[11px] text-muted hover:text-ink">
                ← back to one-click
              </button>
            )}
          </div>
